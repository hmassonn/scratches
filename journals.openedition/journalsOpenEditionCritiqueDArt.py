...
first_page = requests.get('https://journals.openedition.org/critiquedart/66')
...
for one_url in pages_list:
    one_page = requests.get('https://journals.openedition.org/critiquedart/' + one_url)
    ...
    for new_link in links_list:
        print(new_link)
        new_page = requests.get('https://journals.openedition.org/critiquedart/' + new_link)
        domino = html.fromstring(new_page.content)
        new_content = domino.xpath("//div[@id='text']//p[@class='texte']/text()")

        file = open('critiquedart.txt', 'a')
        two_p = ""
